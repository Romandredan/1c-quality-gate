...
| `ai-antipatterns`, `platform-antipatterns` | `tools/catalog.mjs` (аттестация результата читателя) |
...
Четыре штатных, **только читающие**: записи следа они не пишут, агент возвращает факты, запись
формирует контур. Три дешёвые; `antipattern-reader` — на модели уровня контура, не haiku:
семантика антипаттернов LLM-кода не механизируется до дешёвой.
...
| `antipattern-reader` | контур `code`, каталог антипаттернов | кандидаты по индексу триггеров с цитатами; результат аттестует catalog.mjs | один на прогон контура |
