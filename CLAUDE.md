...
**Валидатор не отвергает собственный вывод плагина.** Закрытые списки `evidence-validator.mjs`
(секции настройки, измерения, имена проверок, архетипы) выводятся из источника, по которому
строку печатает инструмент, либо сверяются с ним тестом — своя копия списка с ручной
синхронизацией запрещена. Разошедшийся список запирает гейт у всех проектов, где встретилось
новое имя, и снять его можно только ложной отметкой C0: так в 3.8.0 валидатор не знал раздел
`tests`. Сквозной тест «строка из `gate.mjs plan` → `evidence-validator --gate`» держит эту
границу для каждой секции `DEFAULTS`.
