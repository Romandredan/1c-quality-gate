...
«транзакцией» и «интеграцией». Определяются механически по маркерам в изменённом коде и
путям файлов.
...
У `arch`, в отличие от `code`, объём C2 **сам по себе** минимума не даёт — ячейка «ур. 1–2»
это диапазон, в котором `arch` работает, когда его поднял архетип или сложность, а не
гарантированный пол при любом C2: живой след показывает `arch:skip` ровно при `volume=C2` без
сработавших архетипов. Жёсткий пол в 3 при C3 к тому же перебивал бы собственный `minArch: 2`
архетипа `new-common-module`, из-за которого правка и стала C3.
