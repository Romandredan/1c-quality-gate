...
| `qg:AI-08` | 🟠 | model | always | чтение | Набор функций-помощников достаёт из ответа по одному полю каждая. |
| `qg:AI-09` | 🟠 | model | always | чтение | Чтение константы или поиск предопределённого элемента внутри цикла по строкам. |
| `qg:AI-10` | 🟠 | model | query | чтение | Вложенные циклы поиска соответствий и построение `Соответствие` для связывания наборов из базы. |
...
| `qg:AI-18` | 🟠 | model | query | чтение | Ключ соответствия склеен из значений строкой, либо цикл по коллекции ради поиска по двум-трём полям. |
