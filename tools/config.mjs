...
export const DEFAULTS = {
  ...
  artifacts: { pairs: [] },
  // Пути автотестов. Правка файла по такому пути гейт не взводит вовсе: тестовый код не
  // входит в архитектуру проекта, а его проверка — работа без пользы. Плагин тесты не
  // угадывает — пути называет проект; план лишь подсказывает про модули YAxUnit.
  tests: { paths: [] },
  // Правила `tools/bsl-lint.mjs`, у которых предмет задаёт команда, а не плагин. Слов механики
  // у плагина нет вовсе: какие слова в именах запрещены, решает проект, и пустой список
  // выключает правило. Метки ссылок в комментариях — `null` значит встроенный список, массив
  // регулярных выражений строками заменяет его целиком. Строгий режим менеджера записи
  // выключен: стандарт запись менеджером по полному ключу допускает.
  bslLint: { mechanicWords: [], commentMarkers: null, strictRecordManager: false },
};
    ...
    JSON.stringify(
      {
        ...
        archetypes: {
          '//':
            'custom: архетипы проекта. Запись задаёт ЛИБО "name" (новая метка в дополнение к ' +
            'встроенным — { "name": "имя", "markers": [...], "minCode": "L1|L2", "minArch": "1|2|3" }), ' +
            'ЛИБО "extends" (расширение встроенного архетипа его же меткой — маркеры проекта ' +
            'добавляются к встроенным, minCode/minArch поднимают минимум, но не понижают: ' +
            '{ "extends": "integration", "markers": ["ОбщийМодульHTTPКлиент"] } поднимает "integration" ' +
            'на вызовы общего модуля-обёртки HTTP, которого нет во встроенной таблице). minArch ' +
            'необязателен в обоих случаях.',
        },
        sentinel: {
          '//':
        ...
        tests: {
          '//':
            'paths: пути автотестов от корня проекта — правка файла по такому пути гейт не взводит ' +
            'и ни одним контуром не проверяется. Путь без маски — каталог целиком: ' +
            '["src/cfe/Автотесты"]; маски * и ** тоже работают: ["**/тест_*"]. Исключение видно ' +
            'в следе как config=custom:tests.',
        },
        bslLint: {
          '//':
            'Правила tools/bsl-lint.mjs с предметом от проекта. mechanicWords: начала слов механики, ' +
            'запрещённых в именах ("Кандидат", "Окн" находит "НачалоОкнаСбора"); пустой список ' +
            'выключает правило. commentMarkers: регулярные выражения меток ссылок на задачи и ' +
            'документы в комментариях; null — встроенный список. strictRecordManager: true — любое ' +
            'СоздатьМенеджерЗаписи() находка (правило «пишем набором, читаем запросом»).',
        },
      },
      null,
      2
