...
- [ ] В комментариях нет истории решения: номеров задач, «решений владельца», ссылок на проектные документы (`qg:BSL-COMMENT-REFERENCE`, прогоняется `tools/bsl-lint.mjs` по методам из правки; рассуждение без меток остаётся глазам).
...
## 2. Именование
- [ ] Имена из предметной области, а не механики алгоритма (#std647). Слова механики из списка проекта отмечает `qg:BSL-MECHANIC-WORD` (прогоняется `tools/bsl-lint.mjs` по методам из правки, список — `bslLint.mechanicWords`); главное — не подбирается имя, значит метод режется иначе — остаётся глазам.
...
- [ ] Поля не переносятся поштучно из одного источника в один приёмник: одноимённые — `ЗаполнитьЗначенияСвойств`, строка выборки с полями под имена приёмника — передаётся как есть (`qg:BSL-FIELD-TRANSFER`, прогоняется `tools/bsl-lint.mjs` по методам из правки).
...
- [ ] Реквизиты объекта, найденного запросом, выбираются в том же запросе, а не отдельным `ЗначенияРеквизитовОбъекта` по полученной ссылке (`qg:BSL-QUERIED-OBJECT-ATTRIBUTES`, прогоняется `tools/bsl-lint.mjs`; вызовы из других модулей инструмент не прослеживает).
- [ ] `ВыполнитьПакет()` стоит там, где нужны два результата и больше; ради последнего результата-выборки хватает `Выполнить()`. Индекс пакета постоянного состава — числом, не от `ВГраница()` (`qg:BSL-BATCH-ONE-RESULT`, прогоняется `tools/bsl-lint.mjs`).
...
- [ ] РегистрСведенийМенеджерЗаписи применён верно (#std447). Запись, которую не меняют, читается запросом, а не менеджером с `Прочитать()` (`qg:BSL-RECORD-MANAGER-READ-ONLY`, прогоняется `tools/bsl-lint.mjs`).
