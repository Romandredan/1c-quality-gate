...
`query-alias-shadowing`, `query-alias-vs-nested-table`, `query-top-order`, `transaction-nesting`,
`enum-string-assign`, `unbounded-string-column`, `attribute-access`, `form-attribute-shadowing`,
`stale-local-calls`, `file-encoding`, `registration-check`, `uuid-uniqueness`,
`structure-validation`, `form-binding`. Отметка должна быть не старше
