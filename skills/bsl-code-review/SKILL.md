...
| `qg:BSL-DB-READ-IN-LOOP` | 🟠 | чтение базы, достижимое из тела цикла через вызов метода: тот же N+1, что у запроса в цикле (`references/catalog/BSL-QUERY-IN-LOOP.md`), только распределённое по методам (#std436). Прямую форму ловит анализатор | `references/catalog/BSL-DB-READ-IN-LOOP.md` |
