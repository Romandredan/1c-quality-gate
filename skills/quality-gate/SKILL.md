...
9. **План считает и исполняет `gate.mjs run`**; модель вправе поднять глубину с причиной, понизить —
   нет.
...
прогона, не в контексте. `--no-analyzer` — когда анализатор недоступен или падает; ось
сложности тогда `not_computed`.
...
Их исполнил `run`. Строки следа печатают сами инструменты — переноси черновик дословно, не
сочиняй. **СБОЙ** в сводке: строки за упавший инструмент нет — перезапусти его (`--only`) либо
закрой `skipped` с причиной. После правки кода повтори `run`. `platform-context-run.mjs` обязателен и при `--only`:
анализатор знает имена конфигурации, но не платформы. Субагентам называй каталог
прогона: готовый вывод они читают оттуда, а не гоняют инструменты заново.

Прогон отмечается в журнале `qg-runs.jsonl`: валидатор сверяет по нему каждую запись
`applied` — инструмент видел **весь** состав правки; `skipped ... reason=not_applicable` —
тоже утверждение о работе. Прогоны по частям складываются.
...
**Только читающие**: возвращают факты, запись формирует контур. Дешёвые, кроме
`antipattern-reader` и `cold-reader`: семантика и суждение до haiku не сводятся.

| Субагент | Вызывает | Факты | Спавнов |
|---|---|---|---|
| `bsl-verifier` | `code` | сигнатуры платформы, экспортность модулей, метаданные | на файлы |
| `antipattern-reader` | `code` | кандидаты с цитатами; аттестует `tools/catalog.mjs` | на контур |
| `cold-reader` | `code` | слой 2: чтение без замысла | на контур |
