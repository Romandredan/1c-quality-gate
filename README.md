...
flowchart TD
    ...
    STOP -->|да| PLAN{{"gate.mjs plan:<br/>профиль по трём осям, строка scope,<br/>команды инструментов, справочники под архетип"}}
...
2. `gate.mjs plan` печатает профиль по трём осям, готовую строку `scope` (с `config=...`),
   команды инструментов по порядку выполнения и справочники, которые открывает сработавший
   архетип. Модель вправе поднять глубину против расчётной с записанной причиной, понизить —
...
node "$QG/tools/gate.mjs" plan --files <f> [<f> ...]               # профиль, scope, команды инструментов и справочники под архетип
