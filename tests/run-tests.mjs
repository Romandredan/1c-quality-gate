...
// ---------------------------------------------------------------------------
section('Перенос полей поштучно из одного источника в один приёмник');

// Случай из практики: параметры постановки в очередь переписаны в менеджер записи поле за
// полем. Поле, добавленное в параметры, до регистра не доезжает, и сообщения об этом нет.
// Фикстуры лежат вне git — сравнивать с HEAD не с чем, и проверяется модуль целиком.
{
  const f = writeBytes('transfer-same/Module.bsl', [
    'Процедура ПоставитьВОчередь(ПараметрыПостановки) Экспорт',
    '\tМенеджерЗаписи = РегистрыСведений.ОчередьЗагрузки.СоздатьМенеджерЗаписи();',
    '\tМенеджерЗаписи.Идентификатор = ПараметрыПостановки.Идентификатор;',
    '\tМенеджерЗаписи.Операция      = ПараметрыПостановки.Операция;',
    '\tМенеджерЗаписи.Основание     = ПараметрыПостановки.Основание;',
    '\tМенеджерЗаписи.Склад         = ПараметрыПостановки.Склад;',
    '\tМенеджерЗаписи.Статус        = Перечисления.СтатусыОчереди.Новый;',
    '\tМенеджерЗаписи.Записать();',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('одноимённый перенос четырёх полей — находка', r.out.includes('[qg:BSL-FIELD-TRANSFER] '), r.out.trim().slice(0, 240));
  check('совет — ЗаполнитьЗначенияСвойств с именами источника и приёмника',
    r.out.includes('ЗаполнитьЗначенияСвойств(МенеджерЗаписи, ПараметрыПостановки)'), r.out.trim().slice(0, 400));
  check('присваивание из перечисления в счёт не идёт', r.out.includes('4 присваиваний'), r.out.trim().slice(0, 300));
  check('находка не блокирующая (код 1)', r.code === 1, `код ${r.code}`);
  check('вердикт переноса ушёл в след',
    r.out.includes('scope=field-transfer, ids=[qg:BSL-FIELD-TRANSFER], verdict=violation:qg:BSL-FIELD-TRANSFER'),
    r.out.trim().slice(-600));
}
{
  // Разноимённый перенос из строки выборки: одно значение ходит под двумя именами. Совет —
  // переименовать поле в запросе, а не ЗаполнитьЗначенияСвойств, который разные имена не свяжет.
  const f = writeBytes('transfer-renamed/Module.bsl', [
    'Функция ПоставитьОжидание(Строка)',
    '\tПараметры = ПараметрыПостановки();',
    '\tПараметры.Идентификатор = Строка.Идентификатор;',
    '\tПараметры.Операция      = Строка.Направление;',
    '\tПараметры.ВнешнийСервис = Строка.Протокол;',
    '\tВозврат Параметры;',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('разноимённый перенос — находка с парами имён',
    r.out.includes('[qg:BSL-FIELD-TRANSFER] ') && r.out.includes('Протокол → ВнешнийСервис'), r.out.trim().slice(0, 400));
  check('совет — переименовать поле у источника', r.out.includes('назовите поле в запросе именем приёмника'),
    r.out.trim().slice(0, 500));
}
{
  // `Вставить` — та же форма переноса: ключ литералом, значение полем источника.
  const f = writeBytes('transfer-insert/Module.bsl', [
    'Функция ПараметрыФормыЗаказа(Данные)',
    '\tРезультат = Новый Структура;',
    '\tРезультат.Вставить("Организация", Данные.Организация);',
    '\tРезультат.Вставить("Склад", Данные.Склад);',
    '\tРезультат.Вставить("Контрагент", Данные.Контрагент);',
    '\tВозврат Результат;',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('перенос через Вставить с ключом-литералом — находка', r.out.includes('[qg:BSL-FIELD-TRANSFER] '),
    r.out.trim().slice(0, 240));
}

// Контр-сигналы переноса.
{
  // Преобразование при переносе: ЗаполнитьЗначенияСвойств такие строки не заменит — вопрос.
  const f = writeBytes('transfer-transformed/Module.bsl', [
    'Процедура ЗаполнитьНастройки(Настройки, Реквизиты)',
    '\tНастройки.Адрес = СокрЛП(Реквизиты.Адрес);',
    '\tНастройки.Логин = СокрЛП(Реквизиты.Логин);',
    '\tНастройки.Таймаут = Число(Реквизиты.Таймаут);',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('перенос с преобразованием — вопрос, а не предупреждение',
    r.out.includes('ВОПРОС') && r.out.includes('[qg:BSL-FIELD-TRANSFER] ') && !r.out.includes('ВНИМАНИЕ'), r.out.trim().slice(0, 300));
}
{
  // Два адресата с разными словарями полей — законная форма, пояснённая комментарием.
  const f = writeBytes('transfer-dictionary/Module.bsl', [
    'Процедура ЗаполнитьКоманду(Команда, Строка)',
    '\t// Поля отчёта названы по предметной области, у регистра команд свой словарь.',
    '\tКоманда.СуммаКоманды = Строка.СуммаДеньгами;',
    '\tКоманда.Основание    = Строка.Заказ;',
    '\tКоманда.Склад        = Строка.СкладОтгрузки;',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('комментарий о двух словарях — не находка', !r.out.includes('[qg:BSL-FIELD-TRANSFER] '), r.out.trim().slice(0, 240));
}
{
  // Одно поле в разных ветках — один перенос, порог сам с собой не набирается.
  const f = writeBytes('transfer-branches/Module.bsl', [
    'Процедура ЗаполнитьИсход(Исход, Результат)',
    '\tЕсли Результат.Код = 200 Тогда',
    '\t\tИсход.ТекстОшибки = Результат.ТекстОшибки;',
  ...
  const r = run('tools/bsl-lint.mjs', [f]);
  check('одно поле в трёх ветках — не находка', !r.out.includes('[qg:BSL-FIELD-TRANSFER] '), r.out.trim().slice(0, 240));
}
{
  // Сравнения в условиях — не присваивания; два переноса — ниже порога.
  const f = writeBytes('transfer-compare/Module.bsl', [
    'Функция Совпадает(Запись, Параметры)',
    '\tЕсли Запись.Склад = Параметры.Склад',
    '\t\tИ Запись.Основание = Параметры.Основание',
    '\t\tИ Запись.Сумма = Параметры.Сумма Тогда',
    '\t\tВозврат Истина;',
    '\tКонецЕсли;',
    '\tЗапись.Склад = Параметры.Склад;',
    '\tЗапись.Сумма = Параметры.Сумма;',
    '\tВозврат Ложь;',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('сравнения в условии и два переноса — не находка', !r.out.includes('[qg:BSL-FIELD-TRANSFER] '), r.out.trim().slice(0, 240));
}
{
  // Сравнение, присвоенное переменной, — тоже не перенос: `Б.Х = А.Х` стоит не в начале оператора.
  const f = writeBytes('transfer-boolean/Module.bsl', [
    'Функция ТеЖеПараметры(Запись, Параметры)',
    '\tСкладСовпадает = Запись.Склад = Параметры.Склад;',
    '\tОснованиеСовпадает = Запись.Основание = Параметры.Основание;',
    '\tСуммаСовпадает = Запись.Сумма = Параметры.Сумма;',
    '\tВозврат СкладСовпадает И ОснованиеСовпадает И СуммаСовпадает;',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('сравнения, присвоенные переменным, — не находка', !r.out.includes('[qg:BSL-FIELD-TRANSFER] '), r.out.trim().slice(0, 240));
}
{
  // Методы, которых правка не касалась, не проверяются: в типовом коде перенос поштучно —
  ...
  const repo = join(WORK, 'transfer-repo');
  rmSync(repo, { recursive: true, force: true });
  mkdirSync(repo, { recursive: true });
  const modFile = join(repo, 'Module.bsl');
  const legacy = [
    'Процедура СтарыйПеренос(Приемник, Источник)',
    '\tПриемник.А = Источник.А;',
    '\tПриемник.Б = Источник.Б;',
    '\tПриемник.В = Источник.В;',
    'КонецПроцедуры',
    '',
  ].join('\n');
  writeFileSync(modFile, legacy, 'utf8');
  let gitOk = true;
  try {
    git(repo, 'init', '-q');
  ...
  if (!gitOk) {
    check('git доступен для проверки переноса по методам правки', !process.env.CI, 'git недоступен');
  } else {
    const untouched = run('tools/bsl-lint.mjs', [modFile]);
    check('перенос в методе, которого правка не касалась, — не находка',
      !untouched.out.includes('[qg:BSL-FIELD-TRANSFER] '), untouched.out.trim().slice(0, 240));
    writeFileSync(modFile, legacy + [
      'Процедура НовыйПеренос(Приемник, Источник)',
      '\tПриемник.Г = Источник.Г;',
      '\tПриемник.Д = Источник.Д;',
      '\tПриемник.Е = Источник.Е;',
      'КонецПроцедуры',
      '',
    ].join('\n'), 'utf8');
    const added = run('tools/bsl-lint.mjs', [modFile]);
    const hits = (added.out.match(/\[qg:BSL-FIELD-TRANSFER\] /g) || []).length;
    check('новый метод с переносом — находка, старый по-прежнему молчит',
      hits === 1 && added.out.includes('Г, Д, Е'), added.out.trim().slice(0, 300));
  }
}

// ---------------------------------------------------------------------------
section('Пакет запросов ради одного результата');

// Случай из практики: семь запросов пакета, из которых нужен последний. Выполнить() вернул бы
// его сам, а индекс 6 разъедется с текстом при первой вставке запроса.
{
  const f = writeBytes('batch-last/Module.bsl', [
    'Функция ЗаказыКОплате() Экспорт',
    '\tЗапрос = Новый Запрос(ТекстЗапросаЗаказов());',
    ...
    '\tВозврат',
    '\t"ВЫБРАТЬ Заказы.Ссылка КАК Заказ ПОМЕСТИТЬ ВТЗаказы ИЗ Документ.Заказ КАК Заказы',
    '\t|;',
    '\t|ВЫБРАТЬ Оплаты.Заказ КАК Заказ ПОМЕСТИТЬ ВТОплаты ИЗ РегистрНакопления.Оплаты КАК Оплаты',
    '\t|;',
    '\t|ВЫБРАТЬ ВТЗаказы.Заказ КАК Заказ ИЗ ВТЗаказы КАК ВТЗаказы";',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('последний запрос пакета — единственный результат: находка', r.out.includes('[qg:BSL-BATCH-ONE-RESULT] '),
    r.out.trim().slice(0, 300));
  check('названы номер и размер пакета, совет — Выполнить()',
    r.out.includes('Пакет[2] из 3') && r.out.includes('Запрос.Выполнить()'), r.out.trim().slice(0, 400));
  check('вердикт пакета ушёл в след',
    r.out.includes('scope=batch-single-result, ids=[qg:BSL-BATCH-ONE-RESULT], verdict=violation:qg:BSL-BATCH-ONE-RESULT'),
    r.out.trim().slice(-600));
}
{
  // Индекс от границы у пакета постоянного состава: инструмент сам считает номер.
  const f = writeBytes('batch-upper/Module.bsl', [
    'Процедура Собрать(Результат)',
    '\tЗапрос = Новый Запрос;',
    '\tЗапрос.Текст =',
    '\t"ВЫБРАТЬ 1 КАК А ПОМЕСТИТЬ ВТ1',
    '\t|;',
    '\t|ВЫБРАТЬ ВТ1.А КАК А ИЗ ВТ1 КАК ВТ1',
    '\t|;',
    '\t|ВЫБРАТЬ 2 КАК Б";',
    '\tПакет = Запрос.ВыполнитьПакет();',
    '\tРезультат.Диагностика = Пакет[Пакет.ВГраница() - 1].Выгрузить();',
    '\tРезультат.Данные = Пакет[Пакет.ВГраница()].Выгрузить();',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('индекс от границы у пакета постоянного состава — находка с номером',
    r.out.includes('[qg:BSL-BATCH-ONE-RESULT] ') && r.out.includes('это Пакет[1]'), r.out.trim().slice(0, 400));
  check('два обращения — совета про Выполнить() нет', !r.out.includes('выполняет все запросы пакета'),
    r.out.trim().slice(0, 400));
}
{
  // Литерал прямо в конструкторе: в маске он погашен, и состав обязан считаться по исходнику.
  const f = writeBytes('batch-ctor/Module.bsl', [
    'Функция Остатки()',
    '\tЗапрос = Новый Запрос(',
    '\t"ВЫБРАТЬ 1 КАК А ПОМЕСТИТЬ ВТ',
    '\t|;',
    '\t|ВЫБРАТЬ ВТ.А КАК А ИЗ ВТ КАК ВТ");',
    '\tПакет = Запрос.ВыполнитьПакет();',
    '\tВозврат Пакет[Пакет.ВГраница()].Выгрузить();',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('литерал в конструкторе посчитан: находка, а не вопрос',
    r.out.includes('ВНИМАНИЕ') && r.out.includes('[qg:BSL-BATCH-ONE-RESULT] ') && r.out.includes('из 2'), r.out.trim().slice(0, 300));
}
{
  // Текст склеен из частей — состав не посчитать, находка становится вопросом.
  const f = writeBytes('batch-concat/Module.bsl', [
    'Функция Данные(ТекстОтбора)',
    '\tЗапрос = Новый Запрос;',
    '\tЗапрос.Текст = "ВЫБРАТЬ 1 КАК А ПОМЕСТИТЬ ВТ;" + ТекстОтбора;',
    '\tПакет = Запрос.ВыполнитьПакет();',
    '\tВозврат Пакет[Пакет.ВГраница()].Выгрузить();',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('склеенный текст — вопрос, а не предупреждение',
    r.out.includes('ВОПРОС') && r.out.includes('[qg:BSL-BATCH-ONE-RESULT] ') && !r.out.includes('ВНИМАНИЕ'), r.out.trim().slice(0, 300));
}

// Контр-сигналы пакета.
{
  // Нужный результат не последний, либо последний запрос уничтожает таблицу: Выполнить()
  // вернул бы не то.
  const f = writeBytes('batch-not-last/Module.bsl', [
    'Функция ПервыйРезультат()',
    '\tЗапрос = Новый Запрос(',
    '\t"ВЫБРАТЬ 1 КАК А ПОМЕСТИТЬ ВТ',
    '\t|;',
    '\t|ВЫБРАТЬ ВТ.А КАК А ИЗ ВТ КАК ВТ',
    ...
    '\t|УНИЧТОЖИТЬ ВТ");',
    '\tПакет = Запрос.ВыполнитьПакет();',
    '\tВозврат Пакет[1].Выгрузить();',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('результат не последний (за ним УНИЧТОЖИТЬ) — не находка', !r.out.includes('[qg:BSL-BATCH-ONE-RESULT] '),
    r.out.trim().slice(0, 240));
}
{
  // Пакет перебирается циклом либо возвращается — какие элементы нужны, решает не этот метод.
  const f = writeBytes('batch-escapes/Module.bsl', [
    'Функция ВсеРезультаты()',
    '\tЗапрос = Новый Запрос("ВЫБРАТЬ 1 КАК А ПОМЕСТИТЬ ВТ; ВЫБРАТЬ ВТ.А КАК А ИЗ ВТ КАК ВТ");',
    '\tПакет = Запрос.ВыполнитьПакет();',
    '\tПоследний = Пакет[1];',
    '\tВозврат Пакет;',
    'КонецФункции',
    ...
    '\t\tСообщить(Пакет[1].Пустой());',
    '\tКонецЦикла;',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('пакет возвращается или перебирается циклом — не находка', !r.out.includes('[qg:BSL-BATCH-ONE-RESULT] '),
    r.out.trim().slice(0, 240));
}

// ---------------------------------------------------------------------------
section('Менеджер записи регистра сведений только для чтения');

{
  const f = writeBytes('manager-read/Module.bsl', [
    'Функция ОснованиеВРеестре(Идентификатор, ВидОперации)',
  ...
  const r = run('tools/bsl-lint.mjs', [f]);
  check('менеджер записи только читается — находка',
    r.out.includes('[qg:BSL-RECORD-MANAGER-READ-ONLY] ') && r.out.includes('«РеестрОпераций»'), r.out.trim().slice(0, 300));
  check('вердикт менеджера ушёл в след',
    r.out.includes('scope=record-manager-read, ids=[qg:BSL-RECORD-MANAGER-READ-ONLY], verdict=violation:qg:BSL-RECORD-MANAGER-READ-ONLY'),
    r.out.trim().slice(-600));
}
{
  // Менеджер возвращается наружу: записать его может вызывающий. Вопрос с именем вызывающего.
  const f = writeBytes('manager-returned/Module.bsl', [
    'Функция ЗаписьОчереди(Ключ)',
    ...
    'КонецФункции',
    '',
    'Процедура ЗакрытьЗапись(Ключ)',
    '\tЗапись = ЗаписьОчереди(Ключ);',
    '\tЗапись.Статус = 1;',
    '\tЗапись.Записать();',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('менеджер возвращается — вопрос с вызывающим',
    r.out.includes('ВОПРОС') && r.out.includes('[qg:BSL-RECORD-MANAGER-READ-ONLY] ') && r.out.includes('ЗакрытьЗапись'),
    r.out.trim().slice(0, 400));
}
{
  // Передан аргументом в чужой метод — тоже вопрос; источник ЗаполнитьЗначенияСвойств — нет.
  const f = writeBytes('manager-passed/Module.bsl', [
    'Процедура Обработать(Ключ, Приемник)',
    '\tМенеджер = РегистрыСведений.Очередь.СоздатьМенеджерЗаписи();',
    '\tМенеджер.Ключ = Ключ;',
    '\tМенеджер.Прочитать();',
    '\tЗаполнитьЗначенияСвойств(Приемник, Менеджер);',
    'КонецПроцедуры',
    '',
    'Процедура Передать(Ключ)',
    '\tМенеджер = РегистрыСведений.Очередь.СоздатьМенеджерЗаписи();',
    '\tМенеджер.Ключ = Ключ;',
    '\tМенеджер.Прочитать();',
    '\tОбновитьСтатус(Менеджер, 2);',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  const lines = r.out.split('\n').filter((l) => l.includes('[qg:BSL-RECORD-MANAGER-READ-ONLY] '));
  check('источник ЗаполнитьЗначенияСвойств — предупреждение, передача в метод — вопрос',
    lines.length === 2 && lines.some((l) => l.includes('ВНИМАНИЕ')) && lines.some((l) => l.includes('ВОПРОС')),
    lines.join(' | ').slice(0, 400));
}
{
  // Контр-сигналы: запись после чтения, изменение ресурса после чтения.
  const f = writeBytes('manager-write/Module.bsl', [
    ...
    'КонецПроцедуры',
    '',
    'Процедура Отметить(Ключ, Результат)',
    '\tМенеджер = РегистрыСведений.Очередь.СоздатьМенеджерЗаписи();',
    '\tМенеджер.Ключ = Ключ;',
    '\tМенеджер.Прочитать();',
    '\tЕсли Менеджер.Статус = 3 Тогда',
    '\t\tМенеджер.Статус = 4;',
    '\tКонецЕсли;',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('менеджер пишется или меняется после чтения — не находка', !r.out.includes('[qg:BSL-RECORD-MANAGER-READ-ONLY] '),
    r.out.trim().slice(0, 240));
}
{
  // Записи следа трёх правил — и чистые, и с нарушением — принимает валидатор.
  const clean = writeBytes('lesson-rules-evidence/Module.bsl', 'Процедура Пустая()\n\tА = 1;\nКонецПроцедуры\n');
  const dirty = join(WORK, 'manager-read', 'Module.bsl');
  for (const [name, file] of [['чистые', clean], ['с нарушением', dirty]]) {
    const printed = run('tools/bsl-lint.mjs', [file]).out.split('## quality evidence')[1]?.trim() || '';
    const report = writeBytes(`ev-from-lesson-rules-${name === 'чистые' ? 'clean' : 'dirty'}.md`,
      '## quality evidence\n\n' +
      '[qg scope: volume=C1, files=1, archetypes=[none], driver=volume, resolved=code:L1, config=default]\n' +
      '[qg sentinel: target=v8std, id=std454, status=found]\n' +
      printed + '\n' +
      ...
      CATALOG_DECLARED);
    const r = run('tools/evidence-validator.mjs', [report, '--gate']);
    check(`записи следа переноса, пакета и менеджера (${name}) проходят валидатор`, r.code === 0,
      `${printed.slice(-400)} → ${r.out.trim().slice(0, 300)}`);
  }
}

// ---------------------------------------------------------------------------
section('Реквизиты объекта, найденного запросом, читаются отдельно');

// Случай из практики: функция модуля находит учётную запись запросом, а реквизиты этой
// записи читает помощник библиотеки в следующем методе — два обращения там, где хватило бы одного.
{
  const f = writeBytes('queried-attrs-chain/Module.bsl', [
    'Функция НастройкиПоСкладу(Склад) Экспорт',
    '\tЗаписи = УчетныеЗаписиСклада(Склад);',
    '\tЕсли Записи.Количество() = 0 Тогда',
    '\t\tВозврат Неопределено;',
    '\tКонецЕсли;',
    '\tВозврат НастройкиПоЗаписи(Записи[0]);',
    'КонецФункции',
    '',
    'Функция УчетныеЗаписиСклада(Склад)',
    '\tЗапрос = Новый Запрос("ВЫБРАТЬ Т.Ссылка КАК Ссылка ИЗ Справочник.УчетныеЗаписи КАК Т ГДЕ Т.Склад = &Склад");',
    '\tЗапрос.УстановитьПараметр("Склад", Склад);',
    '\tВозврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");',
    'КонецФункции',
    '',
    'Функция НастройкиПоЗаписи(УчетнаяЗапись)',
    '\tВозврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, "Адрес, Логин");',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('реквизиты ссылки из запроса через параметр — находка с вызывающим',
    r.out.includes('[qg:BSL-QUERIED-OBJECT-ATTRIBUTES] ') && r.out.includes('НастройкиПоСкладу'), r.out.trim().slice(0, 400));
  check('вердикт ушёл в след',
    r.out.includes('scope=queried-object-attributes, ids=[qg:BSL-QUERIED-OBJECT-ATTRIBUTES], verdict=violation:qg:BSL-QUERIED-OBJECT-ATTRIBUTES'),
    r.out.trim().slice(-700));
}
{
  // В том же методе: выборка запроса, затем чтение реквизитов по её полю.
  const f = writeBytes('queried-attrs-local/Module.bsl', [
    'Функция ДанныеДоговора(Контрагент)',
    '\tЗапрос = Новый Запрос("ВЫБРАТЬ Т.Ссылка КАК Договор ИЗ Справочник.Договоры КАК Т ГДЕ Т.Владелец = &К");',
    '\tЗапрос.УстановитьПараметр("К", Контрагент);',
    '\tВыборка = Запрос.Выполнить().Выбрать();',
    '\tЕсли Не Выборка.Следующий() Тогда',
    '\t\tВозврат Неопределено;',
    '\tКонецЕсли;',
    '\tВозврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Договор, "Валюта");',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('реквизит поля выборки в том же методе — находка', r.out.includes('[qg:BSL-QUERIED-OBJECT-ATTRIBUTES] '),
    r.out.trim().slice(0, 300));
}
{
  // Объект пришёл параметром извне модуля: запроса, который его нашёл, здесь нет.
  const f = writeBytes('queried-attrs-param/Module.bsl', [
    'Функция ВалютаДоговора(Договор) Экспорт',
    '\tВозврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "Валюта");',
    'КонецФункции',
    '',
    'Функция ВалютаПоУмолчанию(Настройки)',
    '\tВозврат ВалютаДоговора(Настройки.Договор);',
    'КонецФункции',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('ссылка из параметра извне модуля — не находка', !r.out.includes('[qg:BSL-QUERIED-OBJECT-ATTRIBUTES] '),
    r.out.trim().slice(0, 300));
}

// ---------------------------------------------------------------------------
section('Состав структуры проверяется списком полей');

{
  const f = writeBytes('own-structure/Module.bsl', [
    'Процедура ПроверитьПараметры(Параметры)',
    '\tОбязательныеПоля = "Идентификатор,Операция,Основание,Склад";',
  ...
  const r = run('tools/bsl-lint.mjs', [f]);
  check('Свойство в цикле по списку полей — вопрос',
    r.out.includes('ВОПРОС') && r.out.includes('[qg:BSL-OWN-STRUCTURE-CHECK] '), r.out.trim().slice(0, 300));
  check('вердикт ушёл в след',
    r.out.includes('scope=own-structure-check, ids=[qg:BSL-OWN-STRUCTURE-CHECK], verdict=violation:qg:BSL-OWN-STRUCTURE-CHECK'),
    r.out.trim().slice(-700));
}
{
  // Литерал прямо в вызове — тот же вопрос.
  const f = writeBytes('own-structure-literal/Module.bsl', [
    'Функция Полная(Данные)',
    '\tДля Каждого Поле Из СтрРазделить("Код,Сумма", ",") Цикл',
  ...
  const r = run('tools/bsl-lint.mjs', [f]);
  check('список полей литералом в вызове — вопрос', r.out.includes('[qg:BSL-OWN-STRUCTURE-CHECK] '), r.out.trim().slice(0, 300));
}
{
  // Цикл по списку, собранному не литералом, или без Свойство в теле — не вопрос.
  const f = writeBytes('own-structure-clean/Module.bsl', [
    'Процедура Разобрать(Данные, СписокПолей)',
    '\tДля Каждого Поле Из СтрРазделить(СписокПолей, ",") Цикл',
    '\t\tЕсли Не Данные.Свойство(Поле) Тогда',
    '\t\t\tПродолжить;',
    '\t\tКонецЕсли;',
    '\tКонецЦикла;',
    '\tДля Каждого Код Из СтрРазделить("А,Б", ",") Цикл',
    '\t\tСообщить(Код);',
    '\tКонецЦикла;',
    'КонецПроцедуры',
  ].join('\n'));
  const r = run('tools/bsl-lint.mjs', [f]);
  check('список не литералом и цикл без Свойство — не вопрос', !r.out.includes('[qg:BSL-OWN-STRUCTURE-CHECK] '),
    r.out.trim().slice(0, 300));
}
...
// Модель зафиксирована нижней границей: `inherit` на дешёвой сессии дал бы дешёвого читателя.
{
  ...
  for (const [needle, label] of [
    ...
    ['а не перед JSON', 'отчёт о недоступном индексе не ломает формат «один JSON»'],
...

{
  ...
  // Меряется настоящий файл, а не проекция на плановое число карточек: проекция, записанная
  // на время переноса справочников, после его конца занижала размер, как только каталог
  // перерос план. Индекс читается целиком при каждом проходе, поэтому бюджет — его вес.
  //
...
// сторон: что покрыто и что соседнее — нет.
{
  ...
  // Выгрузка кладёт XML модуля рядом с его каталогом: CommonModules/тест_М.xml и
  // CommonModules/тест_М/Ext/Module.bsl. Новый тест взводит гейт именно этим XML, поэтому маска
  // обязана покрывать обоих — как путь без маски покрывает каталог целиком.
  check('маска, совпавшая с каталогом, покрывает всё под ним', hit('src/cfe/Автотесты/CommonModules/тест_М/Ext/Module.bsl', ['**/тест_*']));
  check('маска покрывает XML модуля рядом с его каталогом', hit('src/cfe/Автотесты/CommonModules/тест_М.xml', ['**/тест_*']));
  check('маска по каталогу-предку не цепляет соседа без префикса', !hit('src/cfe/Автотесты/CommonModules/М.xml', ['**/тест_*']));
