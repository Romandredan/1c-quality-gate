...
export function handoffLines({ sessionId, packageRoot, mode = 'claude' }) {
  ...
  return [
    ...
    '',
    `Запусти инструментом ${tool} субагента \`${runnerType({ packageRoot, mode })}\` одним вызовом. Он не видит этой`,
    ...
    '',
    'Он вернёт вердикт, находки и путь к отчёту. Сними гейт по отчёту и покажи пользователю вердикт',
    'и находки 🔴/🟠 — гейт удостоверяет, что проверка прогнана, а не что код чист:',
    `  node "${qg}/tools/gate.mjs" release --session ${sessionId} --evidence <путь к отчёту>`,
    'Исправление по находкам взведёт гейт заново, и проверка повторится тем же способом.',
    '',
