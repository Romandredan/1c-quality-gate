...
Ловит невидимое статике: неверную бизнес-логику, упущенные сценарии и состояния.
Исполнителей два, с противоположными входами — почему, разбирает `references/cold-reader.md`.

| Условие | Исполнитель |
|---|---|
| класс C3 либо затронуты проведение, деньги, права, необратимые операции | субагент `cold-reader` обязательно; `advisor()` — дополнительно, если он есть в среде |
| прочие L2, `advisor()` есть | `advisor()` — видит транскрипт, сверяет обещанное с написанным |
| прочие L2, `advisor()` нет | `cold-reader`; `skipped` законен, только если недоступен и он |

**`cold-reader` получает ровно две вещи:** путь к сохранённому сравнению версий и пути
изменённых файлов. Формулировку задачи, свои выводы и названия найденных проблем не передавай:
узнавший намерение читатель перестаёт быть холодным. Вопросы и формат ответа — в файле агента.

**Модель не ниже модели сессии.** В файле агента стоит `opus`; сессия на модели выше —
передай её параметром `model` при запуске. Расхождение читателя с `advisor()` — сигнал, а
не шум: код допускает два прочтения. Находку читателя без цитаты из файла не принимай.
...
[qg skipped: layer=code, scope=logic-review, reason=reviewer_unavailable]
```
