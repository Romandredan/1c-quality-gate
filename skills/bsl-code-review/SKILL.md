...
`index` и список изменённых `.bsl`. Он не знает задачи и возвращает JSON — сохрани его в
файл: путь к этому файлу и список файлов правки подставляются в строку плана для
`"$QG/tools/catalog.mjs" attest`.
