...
| `qg:BSL-BATCH-ONE-RESULT` | 🟡 | platform | query | `bsl-lint.mjs` | `ВыполнитьПакет()` ради одного последнего результата; индекс от `ВГраница()`. |
| `qg:BSL-COMMENT-REFERENCE` | 🟡 | model | always | `bsl-lint.mjs` | Комментарий ссылается на задачу, решение владельца или проектный документ. |
...
| `qg:BSL-FIELD-TRANSFER` | 🟡 | model | always | `bsl-lint.mjs` | Три и больше `Приемник.Х = Источник.Х` из одного источника в один приёмник. |
...
| `qg:BSL-MECHANIC-WORD` | 🟡 | model | always | `bsl-lint.mjs` | Имя в изменённом методе содержит слово механики из списка проекта. |
...
| `qg:BSL-OWN-STRUCTURE-CHECK` | 🟡 | model | always | `bsl-lint.mjs` | `Свойство(Имя)` в цикле по `СтрРазделить` со списком обязательных полей структуры. |
| `qg:BSL-QUERIED-OBJECT-ATTRIBUTES` | 🟠 | model | query | `bsl-lint.mjs` | `ЗначенияРеквизитовОбъекта` по ссылке, которую только что вернул запрос этого же модуля. |
...
| `qg:BSL-RECORD-MANAGER-READ-ONLY` | 🟡 | platform | always | `bsl-lint.mjs` | `СоздатьМенеджерЗаписи()` и `Прочитать()`, запись не меняется и не пишется. |
