...
- [ ] Набор однородных записей несёт таблица значений, а не массив структур: результат запроса берётся `Выгрузить()`, недостающий признак добавляется колонкой с явным `ОписаниеТипов` (`qg:AI-17`). Разнородные элементы, уход носителя в сериализацию и пересечение границы клиент-сервер — законные исключения.
- [ ] Поиск по нескольким полям — `НайтиСтроки` по структуре отбора и индекс того же состава (#std452), а не склеенный строковый ключ и не отдельная колонка со склейкой (`qg:AI-18`). Доступ по одному ключу на горячем пути — законное `Соответствие`.
