...
if (process.argv[1]?.endsWith('platform-context-run.mjs')) {
  main(process.argv).then((code) => {
    // Не `process.exit(code)`: когда stdout — труба (а он труба всегда, когда инструмент
    // зовёт оркестратор или субагент), запись асинхронная, и мгновенный выход обрывает
    // недописанный поток. На сборочном агенте Windows это выглядело падением уже
    // отработавшего процесса: след напечатан наполовину, код возврата аварийный.
    //
    // Код ставится полем, процесс завершается сам, когда цикл событий опустеет. Страховка
    // на случай, если что-то в цикле осталось: таймер с `unref` его не держит, поэтому в
    // обычном прогоне он не задерживает выход ни на миллисекунду.
