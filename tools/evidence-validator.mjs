 ...
 * новое имя печатает, — иначе валидатор отвергает собственный вывод плагина. Тест сканирует
 * весь состав пакета на `not_verified: dimension=` и сверяет найденное с этим списком.
 */
...
/**
 * Метки архетипов — дублируют `ARCHETYPES` из `tools/profile.mjs` (плюс `none`); тест в
 * `tests/run-tests.mjs` держит оба списка равными.
 *
...
export function validate(text, { gate = false, root = null, session = null } = {}) {
  ...
  //
  // Иначе он остаётся необязательным, а необязательная проверка не выполняется: отчёт без
  // единого упоминания движка неотличим от отчёта, где движок промолчал. Требуется именно
  // ОТЧЁТ, а не успех: `skipped` с причиной — законный исход, его печатает сам инструмент,
  // и сверять код с платформой на машине без 1С нечем. Соблюдение стоит одного прогона.
  //
  // Требование предъявляется только там, где известен состав правки (сессия гейта названа) и
  // где контур кода вообще работал: на правке одних XML модулей нет, и спрашивать не за что.
  if (own?.files?.length) {
    const touchesModules = own.files.some((f) => /\.(bsl|os)$/i.test(f));
    const codeRan = records.some((r) => r.type === 'applied' && String(r.fields.layer || '') === 'code');
    const reported = records.some(
      (r) => String(r.fields.scope || r.fields.dimension || '') === 'platform-api'
    );
    if (touchesModules && codeRan && !reported) {
      add(
        'error',
        0,
        'контур кода отчитался, но о сверке со справочником платформы в следе ничего нет. Её не ' +
          'закрывает ни статический анализатор (он знает имена конфигурации, но не платформы), ни ' +
          'валидаторы XML: запусти tools/platform-context-run.mjs по изменённым модулям и перенеси ' +
          'его строку — движок печатает и `applied`, и `skipped` с причиной сам'
      );
    }
  }

  // `skipped` с причиной `not_applicable`, `no_queries_found` или `unreadable` — тоже
  // утверждение о работе инструмента: кто-то посмотрел файлы и заключил, что правило к ним не
  // относится, запросов в них нет, либо часть не читается. Инструменты такую отметку ставят;
  // без требования она была бы дырой ровно того же вида, что и вердикт без прогона, только
  // шире — ею закрывается любая проверка. `unreadable` печатает `catalog.mjs attest`, когда
  // среди переданных файлов есть настоящие нечитаемые (сам это проверяет, а не берёт заявление
  // читателя на слово) — без журнала эта причина была бы обходом всех остальных проверок:
  // объявил файл нечитаемым и ушёл от сверки цитат. Прочие причины (инструмент недоступен,
  ...
  const claimsRun = [
    ...
    ...records.filter(
      (r) =>
        r.type === 'skipped' &&
        ['not_applicable', 'no_queries_found', 'no_metadata_resolved', 'unreadable'].includes(String(r.fields.reason || ''))
    ),
  ];
