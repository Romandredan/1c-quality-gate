...
/**
 * Канонический ключ файла — та же форма, в какой состав правки хранит гейт: внутри корня
 * проекта относительный путь, ВНЕ корня — абсолютный; слэши вперёд, нижний регистр.
 *
 ...
 *
 * «..» в ключе не бывает. Относительный путь наружу — нестабильный адрес (его смысл зависит
 * от того, где стоит корень), он не совпадает с ключом состояния гейта (там абсолютный
 * путь) и ломает префиксную сверку покрытия. Живой случай: правки соседнего репозитория
 * прошли инструменты с вердиктом clean, но след отклонялся по каждой проверке — журнал
 * говорил `../…`, состояние — `H:/…`, и это были разные имена одного файла.
 *
 * То же правило продублировано в tools/xml/_qg_journal.py (валидаторы XML пишут журнал
 * сами); реализации сверяет межъязыковой тест через `--canon-check`.
 */
export function normalizePath(p, root = projectRoot()) {
  const s = String(p);
  const base = resolve(root);
  const rel = isAbsolute(s) ? relative(base, resolve(s)) : s;
  const unified = rel.split('\\').join('/').replace(/^\.\//, '');
  if (unified === '..' || unified.startsWith('../') || isAbsolute(rel)) {
    return resolve(base, rel).split('\\').join('/').toLowerCase();
  }
