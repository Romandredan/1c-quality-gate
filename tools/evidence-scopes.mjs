...
export const SCOPES = {
  ...
  // Слой 2 контура кода (advisor() и, при высокой цене ошибки, холодный читатель) до этой
  // записи не оставлял следа вовсе: A/B-прогон на живой правке (task-22) показал, что его
  // пропуск на классе C3 не заметил никто — статика прошла, отчёт выглядел полным. Скоуп
  // делает отсутствие видимым: applied с вердиктом обзора либо skipped с причиной.
  'logic-review': {
    layer: 'code',
    tool: null,
    about: 'ревью логики моделью: субагент cold-reader и advisor()',
  },
 ...
 * карточками каталога (`references/catalog/AI-*.md`), инструментальные — со строками,
 * которые печатают сами инструменты.
 *
...
export const QG_IDS = {
  ...
  // Дефекты логики, которые находит слой 2 (advisor() и холодный читатель) и у которых нет
  // своего признака. Без них находка слоя 2 существовала только прозой: в третьем A/B
  // блокирующая находка и ещё одна существенная не имели в следе ни одной записи, и валидатор
  // принял отчёт с нулём предупреждений. Инструментальный признак руками не заявляется даже
  // как нарушение, поэтому дыру в охвате инструмента (текст запроса внутри XML) закрывают эти.
  // CONTRACT — код расходится с заявленным поведением: справкой, комментарием, описанием
  // метода, подписью в интерфейсе. CASE-LOSS — часть допустимых входов молча теряется или
  // обрабатывается не так: неполный разбор значений, отсечение соединением, пустой результат.
  'qg:LOGIC-CONTRACT': { tool: null },
  'qg:LOGIC-CASE-LOSS': { tool: null },
  ...
  // --- AI-антипаттерны: состав сверяется тестом с каталогом карточек -------
