...
попаданию, не заранее. **Делегируй субагенту `antipattern-reader`**: передай ему вывод
`index`, список изменённых `.bsl` и `git diff HEAD --` по этим файлам — признаку `qg:AI-11`
(строки индекса с `*`) нужно сравнение версий, а не только текущий код. Он не знает задачи и
возвращает JSON — сохрани его в файл: путь к этому файлу и список файлов правки подставляются
в строку плана для `"$QG/tools/catalog.mjs" attest`.
...
Записи следа этих двух инструментов подчиняются инварианту 2: переноси дословно, своих
находок этого класса не добавляй.
