...
**Сначала проверь, не закрыт ли этот пункт прогоном.** Если в проекте настроен контур
платформенного API (`tools/platform-context-run.mjs`, секция `platformContext`), он сверяет
...
Если контур не настроен, сверяй вручную по справочнику платформы (MCP `bsl-context`,
