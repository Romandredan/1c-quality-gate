...
`archetypes` сработавшей метки называются в предупреждении; пока (переходное окно,
`docs/RELEASING.md`) это `warn`, следующим MINOR — ошибка.
...
печатает строку следа. Без записи об этих проходах валидатор предупреждает, а следующим
релизом откажет. Причина: проходы делает модель, и без записи их пропуск неотличим от
выполнения.
...
негде, а `attest`, получивший заявку об `examined` без проверенного дифа, не отличил бы её от
настоящей проверки. Отсюда флаг `attest --diff <файл>`: путь к `git diff HEAD --` по файлам
прогона, сохранённому оркестратором до делегирования читателю. `attest` проверяет, что файл
непустой и упоминает каждый файл из `--files` — иначе это проблема вызова, а не тихий пропуск.
Без `--diff` карточка с `needs: [diff]` в `examined` не бывает: `attest` отвергает такой
результат («признак qg:AI-11 требует сравнения версий, но `--diff` не передан») и не пишет
журнал.

Сравнивать легитимно нечем — новый файл без истории, репозиторий без git — тогда
`attest --no-diff-available` печатает честную запись вместо молчания:
...
обязана быть одной из двух форм. Пока предупреждение (переходное окно, `docs/RELEASING.md`):
следующим MINOR отсутствие записи на `L2` станет ошибкой строгого режима.
