...
признаку `qg:AI-11` нужно сравнение версий, а взять его некому: у читателя нет оболочки, у
`attest` без `--diff` карточка не попадёт в `examined`. Сравнивать не с чем (новый файл без
истории) — `attest --no-diff-available` печатает честный `skipped` вместо молчания.
