...
Субагент `cold-reader` и `advisor()` (`../../bsl-code-review/SKILL.md`, слой 2) не печатают ничего, кроме того, что впишет в отчёт модель — в отличие от инструментальных
...
Пропуск законен, только когда недоступны оба исполнителя: `advisor()` есть не во всякой среде,
а субагент `cold-reader` работает в любом харнессе, и отсутствие одного `advisor()` слой не
отменяет.
