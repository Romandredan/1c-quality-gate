...
export function renderIndex(cards) {
  const head = [
    ...
    '',
    'Часть уроков — не про код, а про устройство программы: контур `bsl-architecture-review`',
    '(доверие результату собственной функции, одна граница проверки вместо повторных, общий',
    'механизм вместо копии на каждый вариант входа, вынесение подготовки данных в отдельный',
    'метод, поле под будущий признак в записи, функция-конструктор для структуры наружу).',
    '',
    'Регистрация объекта метаданных в составе — контур `xml-structure-review`: файл на диске',
    'без записи в составе не попадает в сборку, а загрузка конфигурации этого не покажет.',
    '',
